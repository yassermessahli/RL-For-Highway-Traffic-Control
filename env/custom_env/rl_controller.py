...
class RLController(SumoEnv):
    def __init__(self, *args, **kwargs):
        ...
        self.observation_space_n = self.MACRO_STATE_SIZE + grid_flat_size
    ...
    def _get_current_observation(self):
        ...
        return np.concatenate((vector_state, flat_grid_state))
    ...
    def reward_merging_throughput(self):
        if self.get_edge_lane_n(self.MERGING_EDGE) > 0:
            max_possible_throughput = self.MAX_LANE_FLOW_VPH * self.get_edge_lane_n(
                self.MERGING_EDGE
            )
        else:
            max_possible_throughput = self.MAX_LANE_FLOW_VPH
        norm_throughput = np.clip(
            self.processed_flow_merging_vph
            / (max_possible_throughput if max_possible_throughput > 0 else 1.0),
            0,
            1,
        )
